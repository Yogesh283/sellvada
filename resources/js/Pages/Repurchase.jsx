...
function Popup({ open, title, message, tone = "neutral", onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div
        className={`w-full max-w-md rounded-2xl bg-white shadow-xl ring ${toneRing} ring-1`}
      >
  ...
  return (
    <div className="grid grid-cols-1 gap-3 py-3 border-b border-slate-100 sm:grid-cols-[88px_1fr_120px_92px_120px_64px] sm:items-center">
      ...
      <div className="flex items-center gap-1">
        <button
          onClick={onDec}
          className="h-7 w-7 rounded-md border bg-white hover:bg-slate-50"
          type="button"
        >
          −
        </button>
        <input
          readOnly
          value={it.qty}
          className="h-7 w-10 rounded-md border text-center text-sm"
        />
        <button
          onClick={onInc}
          className="h-7 w-7 rounded-md border bg-white hover:bg-slate-50"
...
export default function Repurchase({
  ...
  const discount = useMemo(
    () => (appliedCoupon === "FLAT10" ? Math.round(subTotal * 0.1) : 0),
  ...
  const showPopup = (title, message, tone = "success") =>
    setPopup({ show: true, title, message, tone });
  ...
  const onCheckout = () => {
    ...
    if (walletBalance < Number(grand))
      return showPopup(
        ...
        `Need ${formatINR(grand)} in wallet.`,
        "error"
      );

    router.post(
      route("repurchase.checkout"),
      { items, coupon: appliedCoupon },
      {
        preserveScroll: true,
        ...
        onSuccess: () => {
          const orderSummary = items
            .map(
              (it) =>
                `${it.name} x ${it.qty} = ${formatINR(it.price * it.qty)}`
            )
            .join("\n");
          setItems([]);
          setAppliedCoupon(null);
        ...
        onError: () =>
          showPopup("Checkout Failed", "Something went wrong.", "error"),
      }
    );
  };
