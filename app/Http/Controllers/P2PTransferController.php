    ...
    try {
        $result = DB::transaction(function () use ($user, $recipient, $amount, $data) {
            ...
            $senderWallet = DB::table('wallet')
                ->where('user_id', $user->id)
                ->where('type', 'main')       // adjust type if you use different naming
                ->lockForUpdate()
                ->first();
            ...
            // Lock recipient's main wallet row (if missing, we can create one or throw)
            ...
            if (! $recipientWallet) {
                // Option A: create a main wallet for recipient if not present
            ...
            DB::table('wallet')
                ...
                ->update([
                    // subtract amount using DB::raw to avoid race if you prefer, but we have lock
