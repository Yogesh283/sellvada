...
public function show(Request $req)
{
    ...
    $lvlById = DB::table('sell')
        ...
        ->groupBy('buyer_id')
        ->pluck('lvl','buyer_id');
    $toPkg = static fn (int $lvl) => $lvl>=3 ? 'diamond' : ($lvl===2 ? 'gold' : ($lvl===1 ? 'silver' : null));
    foreach ($infoById as $id => &$n) {
        $n['package'] = $toPkg((int)($lvlById[$id] ?? 0));
    }
    ...
    $walk = function ($n, $side=null) use (&$walk,&$counts) {
        ...
        if ($side) {
            ...
            if (!empty($n['package'])) $counts['pkg'][$n['package']]++;
        }
